...
- Local checkout: `$SYMPHONY_REPO`, usually `~/src/openai-symphony`.
- `origin` points to `git@github.com:orocsy/symphony.git`.
- `upstream` points to `https://github.com/openai/symphony`.
- GitHub account has `orocsy/symphony`.
...
Sync upstream into the fork before changing workflow examples:
...
1. Keep Symphony engine code synced from upstream.
2. Keep reusable workflow templates in the fork under `examples/`.
