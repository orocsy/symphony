...
| R3 | all MIUs certified, clean worktree, and `HEAD` exactly equals the final MIU certificate, including after recreation with only durable certificates | `handoff_recovery` with the final handoff gate; never fresh implementation |
| R3a | all MIU certificates remain valid but `HEAD` is newer than the final MIU certificate or the worktree is dirty | fail-closed post-certification evidence gate; no edit, commit, or `handoff.requested` command until review-delta authority is reconstructed |
...
- Pending-MIU commit classification is one immutable post-branch-sync snapshot
  ...
  every MIU is already certified, current `HEAD` exactly equals the final MIU
  certificate, and the worktree is clean, a recreated workspace routes directly to structured
  final-handoff recovery. Its prompt permits only the clean
  push/PR/`handoff.requested` sequence and never commands another MIU edit or
  microcommit. A newer `HEAD` or dirty worktree instead receives a fail-closed
  post-certification evidence gate so an authorized review-rework base/mode
  must be reconstructed before final certification.
