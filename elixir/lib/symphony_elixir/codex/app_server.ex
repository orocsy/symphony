defmodule SymphonyElixir.Codex.AppServer do
  ...
  defp review_rework_developer_instructions do
    ...
    Do not post `@codex review` yourself. After the scoped commit is pushed, request final runtime certification with the exact `handoff.requested` command supplied by the runtime contract; Symphony issues `handoff.ready` and posts the fresh review request.
    If the prompt starts with a dirty/local handoff checkpoint, follow that checkpoint first: inspect only the focused local diff and run contract-declared focused validation. If validation names exact in-scope files/assertions, make that smallest repair before committing; otherwise commit, push, request runtime handoff certification, and leave Linear state transitions to Symphony's review monitor.
