defmodule SymphonyElixir.Config.Schema do
  ...
  defmodule ReviewMonitor do
    ...
    embedded_schema do
      ...
      field(:request_timeout_ms, :integer, default: 30_000)
    end
    ...
    def changeset(schema, attrs) do
      ...
      |> cast(
        attrs,
        [:enabled, :provider, :repo, :states, :rework_state, :request_stale_after_ms, :request_timeout_ms],
        empty_values: []
      )
  ...
  defmodule Agent do
    ...
    embedded_schema do
      ...
      field(:max_failed_worker_retries, :integer, default: 3)
    ...
    def changeset(schema, attrs) do
      ...
      |> validate_number(:max_failed_worker_retries, greater_than_or_equal_to: 0)
  ...
  defmodule Codex do
    ...
    defp validate_command_patterns(changeset) do
      changeset
      |> validate_regex_patterns(:forbidden_command_patterns)
      |> validate_regex_patterns(:safe_command_approval_patterns)
    end

    defp validate_regex_patterns(changeset, field) do
      validate_change(changeset, field, fn ^field, patterns ->
        patterns
        |> Enum.flat_map(fn pattern ->
          case Regex.compile(pattern) do
            {:ok, _regex} -> []
            {:error, reason} -> [{field, "invalid regex #{inspect(pattern)}: #{inspect(reason)}"}]
          end
        end)
      end)
    end
