defmodule SymphonyElixir.PromptBuilder do
  ...
  defp structured_contract_has_pending_miu?(issue, workspace) do
    case RuntimeContract.compile(Map.get(issue, :description)) do
      {:ok, compiled} ->
        ...
        Enum.any?(compiled.miu_ids, &(not MapSet.member?(certified_ids, &1)))
  ...
  defp dirty_validated_review_rework_micro_prompt do
    ...
    - Only fall back to a focused `git diff --no-ext-diff --no-textconv -- <dirty-file>` read or focused validation when `git status` shows different dirty files, staged/unmerged conflict markers, missing validation evidence, or a failed git/push/review command requires a precise blocker. Run status and each diff as separate commands; never join them with `&&`, `||`, `;`, or pipes.
  ...
  defp dirty_validated_handoff_checkpoint(status, event_summary, git_context) do
    ...
    - First action: run `git status --short --branch`, then inspect each focused diff with a separate `git diff --no-ext-diff --no-textconv -- <dirty-file>` command and compare it to the recent passed validation/gate evidence listed above. Never combine checkpoint reads with `&&`, `||`, `;`, or pipes.
  ...
  defp unvalidated_local_handoff_checkpoint(status, git_context) do
    ...
    #{git_context}
    - The commit list and diffstat above are runtime-provided. Run bounded `git log` only when the runtime command policy explicitly advertises it; `git diff --stat`, base-branch diff/history commands, and all other denied history forms remain unavailable.
    - First action: run the smallest validation needed for the changed files listed above, using a separate focused `git diff --no-ext-diff --no-textconv -- <dirty-file>` read only when a dirty file needs inspection. Never combine checkpoint reads with `&&`, `||`, `;`, or pipes.
