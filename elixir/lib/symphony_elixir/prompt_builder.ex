defmodule SymphonyElixir.PromptBuilder do
  ...
  defp standard_review_rework_micro_prompt do
    ...
    - After a code/test edit, run only validation declared by the active runtime contract, then commit and push the same branch. Request final runtime certification with `python3 .codex/delivery/bin/orocsy.py --repo . event append --type handoff.requested --status requested --step final` and end the turn. Symphony independently checks the review delta against MIU write scopes, runs the affected MIU and final validations, issues `handoff.ready`, and requests fresh Codex review.
  ...
  defp dirty_validated_review_rework_micro_prompt do
    ...
    - Stage only the intended dirty product/test files, commit with a concise issue-prefixed message, and push the existing branch to its configured upstream. Then request final runtime certification with `python3 .codex/delivery/bin/orocsy.py --repo . event append --type handoff.requested --status requested --step final` and end the turn.
    - Symphony independently checks the review delta against MIU write scopes, runs the affected MIU and final validations, issues `handoff.ready`, and requests fresh Codex review. Leave the issue non-terminal; a fresh review request is not proof of clean review.
