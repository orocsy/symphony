defmodule SymphonyElixir.ExtensionsAuditTest do
  ...
  test "reports a missing baseline object without cascading tree findings" do
    ...
    git = fn "git", args, _opts ->
      send(self(), {:git_args, args})

      case Enum.drop(args, 2) do
        ["rev-parse", "--show-toplevel"] -> {root <> "\n", 0}
        ["cat-file", "-t", _commit] -> {"fatal: Not a valid object name\n", 128}
      end
    end
  ...
  test "reports a repository tree mismatch" do
    ...
    git = fn "git", args, _opts ->
      case Enum.drop(args, 2) do
        ["rev-parse", "--show-toplevel"] -> {root <> "\n", 0}
        ["cat-file", "-t", _commit] -> {"commit\n", 0}
        ["rev-parse", _tree_expression] -> {wrong_tree <> "\n", 0}
      end
    end
    ...
    git = fn "git", args, _opts ->
      case Enum.drop(args, 2) do
        ...
        ["rev-parse", expression] ->
          if String.ends_with?(expression, ":elixir") do
            {wrong_tree <> "\n", 0}
          else
            {"37a4c6c184db05cd2d59bfc50943979919ec988a\n", 0}
          end
      end
    end
  ...
  test "rejects ordinary ancestry when the baseline is absent" do
    ...
    git = fn "git", args, _opts ->
      case Enum.drop(args, 2) do
        ...
        ["rev-parse", expression] ->
          if String.ends_with?(expression, ":elixir") do
            {"77d9ba67775e6681eb1ad5cf03a019e678a8e941\n", 0}
          else
            {"37a4c6c184db05cd2d59bfc50943979919ec988a\n", 0}
          end
  ...
  test "never invokes fetch checkout merge reset or config" do
    ...
    refute Enum.any?(invocations, fn args -> Enum.any?(forbidden, &(&1 in args)) end)
  end
  ...
  test "reports missing, malformed, and non-mapping manifests before invoking git" do
    ...
    on_exit(fn -> Enum.each([missing_root, malformed_root, scalar_root], &File.rm_rf!/1) end)
  ...
  test "rejects unsupported, missing, and ill-typed manifest fields before invoking git" do
    ...
    on_exit(fn -> Enum.each([unsupported_root, missing_root, invalid_date_root], &File.rm_rf!/1) end)
  ...
  defp collect_git_invocations(acc) do
    receive do
      {:git_invocation, args} -> collect_git_invocations([args | acc])
    after
      0 -> Enum.reverse(acc)
    end
  end
